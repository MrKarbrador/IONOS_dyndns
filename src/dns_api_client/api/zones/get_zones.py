...
def _get_kwargs(
    *,
    ...
    url = "{}/v1/zones".format(client.base_url)
...
def _parse_response(*, response: httpx.Response) -> Optional[Union[List[Error], List[Zone]]]:
...
def _build_response(*, response: httpx.Response) -> Response[Union[List[Error], List[Zone]]]:
    return Response(
...
def sync_detailed(
    *,
    client: Client,
) -> Response[Union[List[Error], List[Zone]]]:
    kwargs = _get_kwargs(
        client=client,
    )
...
def sync(
    *,
    client: Client,
) -> Optional[Union[List[Error], List[Zone]]]:
